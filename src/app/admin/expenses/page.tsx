...
export default function ExpensesPage() {
  ...
  const totals = useMemo(() => {
    ...
    const thisMonth = filtered.filter((d) => {
      const now = new Date();
      return d.expense_date >= new Date(now.getFullYear(), now.getMonth(), 1).toISOString().split("T")[0];
  ...
  const summaryCards = [
    ...
    { label: "This Month", value: formatCurrency(totals.thisMonth), icon: Calendar, gradient: "bg-gradient-to-br from-orange-500 via-amber-400 to-yellow-300" },
    { label: "Categories", value: String(Object.keys(totals.byCategory).length), icon: TrendingUp, gradient: "bg-gradient-to-br from-blue-500 via-indigo-400 to-violet-300" },
    { label: "Total Entries", value: String(filtered.length), icon: ArrowUpDown, gradient: "bg-gradient-to-br from-emerald-500 via-teal-400 to-cyan-300" },
  ];
