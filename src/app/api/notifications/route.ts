...
export const GET = withAuth(async (req, supabase, authUserId) => {
  ...
  else if (unreadOnly) query = query.eq("is_read", false);
...
export const PUT = withAuth(async (req, supabase, authUserId) => {
  // Mark the caller's notifications as read. Read notifications are then
  // deleted immediately so the bell only ever holds unread items.
  const svc = createServiceClient();
  await svc.from("notifications").update({ is_read: true }).eq("user_id", authUserId);
